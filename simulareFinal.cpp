...
public:
    ...
    float takeDamage(int nr, int damage)
    {
        return nr * damage;
    }
};
...
public:
    ...
    float resetNumber(int totalHp, int Hp)
    {
        return int(totalHp/Hp+0.5);
    }
};
...
public:
    ...
    float dealDamage(int nr, int damage)
    {
        return nr * damage;
    }
};
...
int main()
{
    ...
    while(enemyHp>0&&base.hp>0)
    {
        ...
        if(pos<=3)
        {
            base.hp -= base.takeDamage(n.nr, n.damage);
            n.position=3;
        }
        if(pos<=1)
        {
            base.hp -= base.takeDamage(w.nr, w.damage) ;
            w.position = 1;
        }
        if(pos<=7)
        {
            base.hp -= base.takeDamage(a.nr, a.damage);
            a.position = 7;
        }
        if(pos<=5)
        {
            base.hp -= base.takeDamage(c.nr, c.damage);
            c.position = 5;
        }
        if(pos<=0)
        {
            base.hp -= base.takeDamage(m.nr, m.damage);
            m.position = 0;
        }
        ...
        if(pos<=7)
        {
            ...
            {
                mHp -= arch.dealDamage(arch.nr,arch.damage);
                m.nr = m.resetNumber(mHp,m.hp);
            }

            else if (wHp>0&&w.position<=7)
            {
                wHp -= arch.dealDamage(arch.nr,arch.damage);
                w.nr = w.resetNumber(wHp,w.hp);
            }

            else if (nHp>0&&n.position<=7)
            {
                nHp -= arch.dealDamage(arch.nr,arch.damage);
                n.nr = n.resetNumber(nHp,n.hp);
            }

            else if (cHp>0&&c.position<=7)
            {
                cHp -= arch.dealDamage(arch.nr,arch.damage);
                c.nr = c.resetNumber(cHp,c.hp);
            }

            else if (aHp>0&&a.position<=7)
            {
                aHp -= arch.dealDamage(arch.nr,arch.damage);
                a.nr = a.resetNumber(aHp,a.hp);
            }
        }
        ...
        if(pos<=5)
        {
            ...
            {
                mHp -= wizzard.dealDamage(wizzard.nr, wizzard.damage);
                m.nr = m.resetNumber(mHp,m.hp);
            }

            else if (wHp>0&&w.position<=5)
            {
                wHp -= wizzard.dealDamage(wizzard.nr, wizzard.damage);
                w.nr = w.resetNumber(wHp,w.hp);
            }

            else if (nHp>0&&n.position<=5)
            {
                nHp -= wizzard.dealDamage(wizzard.nr, wizzard.damage);
                n.nr = n.resetNumber(nHp,n.hp);
            }

            else if (cHp>0&&c.position<=5)
            {
                cHp -= wizzard.dealDamage(wizzard.nr, wizzard.damage);
                c.nr = c.resetNumber(cHp,c.hp);
            }

            else if (aHp>0&&a.position<=5)
            {
                aHp -= wizzard.dealDamage(wizzard.nr, wizzard.damage);
                a.nr = a.resetNumber(aHp,a.hp);
            }
        }
        ...
        if(pos<=3)
        {
            ...
            {
                mHp -= castle.dealDamage(castle.nr, castle.damage);
                m.nr = m.resetNumber(mHp,m.hp);
            }

            else if (wHp>0&&w.position<=3)
            {
                wHp -= castle.dealDamage(castle.nr, castle.damage);
                w.nr = w.resetNumber(wHp,w.hp);
            }

            else if (nHp>0&&n.position<=3)
            {
                nHp -= castle.dealDamage(castle.nr, castle.damage);
                n.nr = n.resetNumber(nHp,n.hp);
            }

            else if (cHp>0&&c.position<=3)
            {
                cHp -= castle.dealDamage(castle.nr, castle.damage);
                c.nr = c.resetNumber(cHp,c.hp);
            }

            else if (aHp>0&&a.position<=3)
            {
                aHp -= castle.dealDamage(castle.nr, castle.damage);
                a.nr = a.resetNumber(aHp,a.hp);
            }
        ...
        else if (enemyHp<=0) {printf("victory\n"); printf("game lasted: %d seconds",sec);}
